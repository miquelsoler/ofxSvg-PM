...
static void svgtiny_parse_transform_attributes(Poco::XML::Element *node,
		struct svgtiny_parse_state *state);
static svgtiny_code svgtiny_add_path(float *p, char *pathId, unsigned int n,
		struct svgtiny_parse_state *state);
		...
		struct svgtiny_parse_state state)
{
	...
	path_id = (char *)path->getAttribute("id").c_str();
	if (!path_id) {
		state.diagram->error_message = "path: missing id attribute";
		return svgtiny_SVG_ERROR;
	}
...
svgtiny_code svgtiny_add_path(float *p, char *pathId, unsigned int n,
		struct svgtiny_parse_state *state)
{
